document.addEventListener("DOMContentLoaded", async function () {
  const inputOnibus = document.querySelector("#onibus");
  const response = await axios.get("http://localhost:3000/api/onibus");

  response.data.forEach((onibus) => {
    let option = document.createElement("option");
    option.text = onibus.placa; 
    option.value = onibus.id;
    inputOnibus.add(option);
  });
});
